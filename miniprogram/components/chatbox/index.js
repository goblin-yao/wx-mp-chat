  ...
  lifetimes: {
    ...
    detached() {
      try {
        ...
        console.error("chatbox component unload!!!!");
      } catch (error) {
  ...
  methods: {
    updatePromptInfo(_d) {
      ...
      this.setData({
        ...
        scenesText: decodeURIComponent(_d.scenesText),
    ...
    scoreHandler(event) {
      ...
      wx.request({
        ...
        data: {
          ...
          options: {
            promptText: Config.SCENCES_ALL.kouyu[0]["ScorePromptText"].replace(
              "{{TOPIC}}",
              this.data.scenesText
            ),
          },
        },
        method: "POST",
